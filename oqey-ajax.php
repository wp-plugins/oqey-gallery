...
function createoQeyNewThumb() {
    ...
    if($im->img_type=="nextgen"){
        ...
        $img        = OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.$blogfolder.$folder."/galimg/".trim($im->title);
        $imgthmb    = OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.$blogfolder.$folder."/galthmb/".trim($im->title);       
    ...
    if(!empty($galleries)){
	 ...
	 $r .= '<tr id="row_'.$gallery->id.'">
                 <td align="center" width="100" valign="middle" style="height:35px; padding-top:7px;">
                   ...
                   <a href="#preview" class="preview-gallery" id="'.$gallery->id.'"><img src="'.oQeyPluginUrl().'/images/preview_button.png" title="'.__('Click to preview this gallery', 'oqey-gallery').'"/></a>
                 </td>
                 <td align="left" width="50">ID: '.$gallery->id.'</td>
                 <td width="720" valign="middle"><div class="dblclick" id="gall_id_'.$gallery->id.'">'.$gallery->title.'</div></td>
...
$resp["response"] .= '<table width="900" border="0" cellspacing="0" cellpadding="0">
	...
	<input class="button" type="button" name="doapply" id="doapply" value="'.__('Apply', 'oqey-gallery').'"/>
	</div>
    </td>
...
max_file_size : "'.oQeyGetNumericUploadSize().'",
url : "'.WP_PLUGIN_URL.'/oqey-gallery/bcupload.php",
resize : {width : 1200, height : 800, quality : 90},
flash_swf_url : "'.WP_PLUGIN_URL . '/oqey-gallery/js/plupload.flash.swf",
silverlight_xap_url : "'.WP_PLUGIN_URL.'/oqey-gallery/js/plupload.silverlight.xap",
filters : [{title : "Image files", extensions : "jpg,jpeg,gif,png,JPG,JPEG,GIF,PNG"},{title : "Zip files", extensions : "zip"}],
multipart_params : {
...
$resp["response"] .='</td>
        <td width="110" height="50" align="left" valign="middle">
          <div class="uploader" id="flashuploader">
            <input type="button" name="uploadimages" id="uploadimages" value="'.__('Upload', 'oqey-gallery').'" class="button" style="float:left;"/>
            <img src="'.WP_PLUGIN_URL . '/oqey-gallery/images/indicator.gif" id="inicon"/>     
          </div>
         </td>
        </tr>
      </table>';
...
function oQeyGetAllImages(){
   ...
   $galfolder  = oqey_get_gallery_folder($_POST['id']);
   $folder     = $galfolder."/galthmb/";
   $imgFolder  = $galfolder."/galimg/";
   $cropFolder = $imgFolder;
   

   $s                 = $wpdb->get_row( $wpdb->prepare( "SELECT splash_img FROM $oqey_galls WHERE id = %d ", absint($_POST['id']) ));
   $resp["allimages"] = "";
   $preload = array();
   ...
   if($img->img_type=="nextgen"){
     ...
     $iimg      = OQEY_ABSPATH.'/'.$img->img_path.'/'.trim($img->title);
     $imgthumb = OQEY_ABSPATH.'/'.$img->img_path.'/thumbs/thumbs_'.trim($img->title);     
     ...
     $iimg      = OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$cropFolder.trim($img->title);
     $imgthumb = OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.trim($img->title);
     $cropimage      = $oqeyImagesRoot.$cropFolder.$img->title;
     $cropimagethmb  = $oqeyImagesRoot.$folder.$img->title;
   
   }
   
   list($twidth, $theight)   = @getimagesize($iimg);
   list($ttwidth, $ttheight) = @getimagesize($imgthumb);
...
function oQeyOrderAllImages(){
   ...
   $gal_img_id  = absint($_POST['galleryid']);
   ...
   foreach ($r as $position => $item){
      ...
      $wpdb->query( $wpdb->prepare("UPDATE $oqey_images SET img_order = %d WHERE id = %d AND gal_id = %d ", $position, $item, $gal_img_id) );
...
function oQeySaveMusicOrder(){
   ...
   $wpdb->query( $wpdb->prepare( "DELETE FROM $oqey_music_rel WHERE gallery_id = %d ", esc_sql($_POST['mgalleryid']) ));
   $i = 0;
   ...
   if(!empty($_POST['selectedmusic'])){
     ...
     foreach ($_POST['selectedmusic'] as $l => $d){
      ...
      $wpdb->query("INSERT INTO $oqey_music_rel (music_id, gallery_id, mrel_order) VALUES ('".$d['value']."', '".absint($_POST['mgalleryid'])."', '".$i."')");
      $i++;
	   ...
	   echo '<tr>
	              ...
	              <img src="http://oqeysites.com/skinsxml/'.$x->archivename.'/'.$x->archivename.'.jpg" alt="skin" style="border: thin solid rgb(153, 153, 153); margin-left: 15px;" width="150" height="100">
                   ...
                   <h4>'.urldecode($x->title).$info.'</h4>
                       <p>'.urldecode($x->description).'<br/>'.$price.'</p>
                       <p>'.$install.' | '.$preview.'</p>
                   </td>
                 </tr>';
    ...
    if(is_file($sfpath)){ $skoptions = '<a href="#set_skin_options" class="set_skin_options" id="skopt'.$r->id.'" rel="'.$r->folder.'">'.__('Skin Options', 'oqey-gallery').'</a> | '; }else{ $skoptions = ""; }
       ...
       echo '<tr id="skink_tr_'.$r->id.'">
        ...
        if($r->firstrun==0){ 
                ...
                echo '<input name="comkey" class="comkey" type="text" value="'.$r->comkey.'" id="key'.$r->id.'" style="background-color:#CCC;width:190px;"/>';
                echo '<input type="button" name="savekey" class="savekey button" id="'.$r->id.'" value="'.__('Save', 'oqey-gallery').'" style="background-color:#CCC;width:63px;">';
                echo '</p>';
                ...
                echo '<input type="hidden" name="s" value="'.$r->skinid.'" />';
                echo '<input type="hidden" name="skinfolder" value="'.$r->folder.'" />';
...
function oQeyImagePermDelete(){
   ...
   foreach ($all as $imgID){
     ...
     if(is_file(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.trim($title)) ){  
       ...
       unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.trim($title));
       unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galthmb/'.trim($title));
       
       if(is_file(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.trim($title))){ 
           
          unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.trim($title));        
