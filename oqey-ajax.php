...
function oQeyGetAllImages(){
   ...
   if($img->img_type=="nextgen"){
     ...
     if($iv == 'vimeo' || $iv == 'youtube'){
         ...
         $fullimg    = wp_get_attachment_thumb_url( $img->meta_data, 'thumbnail' );
...
function oQeyOrderAllImages(){
   ...
   foreach ($r as $position => $item){
      ...
      if($idi[1]=='wp'){
          ...
          $alttextt = get_post_meta( $idi[0], '_wp_attachment_image_alt', true );
      ...
      $wpdb->query( $wpdb->prepare("UPDATE $oqey_images SET img_order = %d WHERE id = %d AND gal_id = %d ", $position, $idi[0], $gal_img_id) );
   ...
   $xml = @simplexml_load_file('http://oqeysites.com/skinsxml/skins.xml');    
    ...
    if(!empty($xml)){
      ...
      if($e==0){ 
          ...
          echo '<tr><td valign="middle" style="height:30px; padding-left: 20px;">'.__('All available skins are already installed...', 'oqey-gallery').'</tr></td>'; }
          echo '</tbody></table>';	
