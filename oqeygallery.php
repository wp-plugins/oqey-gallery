   ...
   if(is_admin() && 
    ...
    if($_GET['page']=='oQeyGalleries'){
     ...
     /*END*/
      ...
      wp_register_style('swipebox', WP_PLUGIN_URL . '/oqey-gallery/css/swipebox.css');
      wp_enqueue_style('swipebox');
      wp_enqueue_script('swipebox', WP_PLUGIN_URL . '/oqey-gallery/js/swipebox.js', array('jquery') ,"", true);
   ...
   if(isset( $atts['arrows'] ) ){   $arrows        = $atts['arrows']; }else{    $arrows        = 'on'; }
    ...
    foreach($ids as $id) { 
       ...
       if($d<1 || get_option('oqey_noflash_options')=="incolums"){
              $nobject .= '<div style="margin:0 auto 5px auto;"><img src="'.$imgurl.'" alt="'.trim($p->post_excerpt).'"/></div>'."\n";
       }else{
