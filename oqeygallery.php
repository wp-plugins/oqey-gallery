...
function oqey_db_install(){
   global $wpdb, $oqey_db_version;
	...
	$perm->add_cap('oQeyTrash');
        ...
        //$perm->add_cap('oQeyCanManageGalleries');
        /*
        if(!get_option('oqey_width')){
            add_option("oqey_width_details", "pr" ,'', 'no');
        }else{
            add_option("oqey_width_details", "px" ,'', 'no');
        }*/
...
function oqey_check_upgrade(){
    ...
    /*Update the administrator roles*/
    ...
    if ( in_array( strtolower($path['extension']), $ext) ){	
       ...
       if(!$sql=$wpdb->get_row( $wpdb->prepare("SELECT * FROM $oqey_music WHERE link = %s ", $audio_file) ) ){
         ...
         rename($file, $audio_path);
           ...
           $wpdb->query( $wpdb->prepare("UPDATE $oqey_music SET link = %s, path = %s, type = %s WHERE link = %s ", $audio_link, $audio_path, "oqey", $audio_file ));
...
function oqey_init_method() { 
   ...
   if(is_admin() && 
      ...
      wp_enqueue_script('jqueryfarbtastic', WP_PLUGIN_URL . '/oqey-gallery/js/farbtastic.js', array('jquery'),"", false);   
    ...
    if(     $_GET['page']=='oQeyGalleries' || 
            ...
            wp_register_script('oqeybrowserplus', 'http://bp.yahooapis.com/2.4.21/browserplus-min.js');
            wp_enqueue_script('oqeybrowserplus');
            wp_enqueue_script('oqeyplupload', WP_PLUGIN_URL . '/oqey-gallery/js/plupload.full.js', array('jquery'),"", false);        
