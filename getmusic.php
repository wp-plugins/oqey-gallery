...
$id = mysql_real_escape_string($_POST['galleryid']);

$oqey_music = $wpdb->prefix . "oqey_music";
$oqey_music_rel = $wpdb->prefix . "oqey_music_rel";

header('Content-Type: ' . feed_content_type('rss-http') . '; charset=' . get_option('blog_charset'), true);
$r = get_option('siteurl').'/wp-content/oqey_gallery/music/';
...
foreach($mus as $m){ 
$dax .= '<song path="'.urlencode($r.trim($m->link)).'" artist="" title="'.urlencode(trim($m->title)).'"></song>';
}
$dax .= '</songs>';
echo $dax;
}
