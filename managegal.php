...
if(id==2){
		...
		jQuery.post("<?php echo oQeyPluginUrl(); ?>/do.php", { imgalltotrash: names },
        function(data){
		if(names.length>0){
		jQuery("#messages").hide().html(decodeURIComponent('<p class="updated fade">' + data + ' <a href="#undo" id="undoallimages">undo<\/a><\/p>')).fadeIn("slow"); 
			...
			jQuery.post("<?php echo oQeyPluginUrl(); ?>/do.php", { imgallfromtrash: names },
            function(r){
...
jQuery.post("<?php echo oQeyPluginUrl(); ?>/do.php", {data: { newtitle: newtitle } },
 function(html){
         var data = eval('(' + html + ')');
