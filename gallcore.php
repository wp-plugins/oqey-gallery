...
function oQeyGetPreviewGallery(){
    ...
    $show = "yes";
  	...
  	$get_bg_img = $wpdb->get_row("SELECT * FROM $oqey_galls WHERE id ='".$galleryID."'");			  
    $allimgs = $wpdb->get_results("SELECT * FROM $oqey_images WHERE gal_id = '".$galleryID."' ORDER BY img_order ASC");
...
function oQeyGetAllImages(){
   ...
   if (is_plugin_active('oqey-photo-cropper/oqeycropper.php')){
   ...
   $croper ='<a href="#image" onclick="showCropImage(\''.$img->id.'\', \''.$twidth.'\', \''.$theight.'\',\''.$ttwidth.'\', \''.$ttheight.'\', \''.urlencode(trim($oqeyImagesRoot.$cropFolder.$img->title)).'\', \''.urlencode(trim($oqeyImagesRoot.$folder.$img->title)).'\'); return false;">
             <img src="'.WP_PLUGIN_URL.'/oqey-photo-cropper/images/cropperbutton.png" width="14" height="14" alt="image" title="crop image"/></a>';    
    }
...
function oQeyUpdateImageDetails(){
   ...
   $alt_up = $wpdb->query( $wpdb->prepare("UPDATE $oqey_images SET alt = '%s', comments = '%s', img_link = '%s' WHERE id = '%d'", stripslashes_deep(urldecode($_POST['alt'])), stripslashes_deep(urldecode($_POST['comments'])), stripslashes_deep(urldecode($_POST['oqey_image_link'])), $_POST['imgid']) ); 

   $m = "not";
   $c = $wpdb->get_row("SELECT splash_img FROM $oqey_galls WHERE id ='".esc_sql($_POST['galid'])."'");

   if($c->splash_img==$_POST['imgid']){
   $splashk = $wpdb->query( $wpdb->prepare("UPDATE $oqey_galls SET splash_img ='%d', splash_only = '%d' WHERE id = '%d'", 0, 0, $_POST['galid'] ));
   }

   if(!empty($_POST['splash']) && !empty($_POST['splashexclusive']) ){
    
   $splashk = $wpdb->query( $wpdb->prepare("UPDATE $oqey_galls SET splash_img ='%d', splash_only = '%d' WHERE id = '%d'", $_POST['imgid'], 1, $_POST['galid'] ));
   $m = "yes";
   
   }elseif(!empty($_POST['splash'])){	 
   $splashk = $wpdb->query( $wpdb->prepare("UPDATE $oqey_galls SET splash_img ='%d', splash_only = '%d' WHERE id = '%d'", $_POST['imgid'], 0, $_POST['galid'] ));   
   $m = "yes";
   }
...
function oQeySaveSkinKey(){
   ...
   $upd = $wpdb->query( $wpdb->prepare("UPDATE $oqey_skins SET comkey = '%s' WHERE id = '%d'", $_POST['comkey'], $_POST['savekey'] )); 
...
function oQeySkinToTrash(){
   ...
   $s = $wpdb->query( $wpdb->prepare("UPDATE $oqey_skins SET status = '%d' WHERE id = '%d' ", 2, $_POST['movetotrashskin'] ));
...
function oQeySkinFromTrash(){
   ...
   $resp = array();
   $s = $wpdb->query( $wpdb->prepare("UPDATE $oqey_skins SET status = '%d' WHERE id = '%d'", 0, $_POST['undoskin'] ));
...
function oQeySongToTrash(){
   ...
   $s = $wpdb->query( $wpdb->prepare("UPDATE $oqey_music SET status = '%d' WHERE id = '%d' ", 2, $_POST['songtotrash'] ));
...
function oQeySongFromTrash(){
   ...
   $s = $wpdb->query( $wpdb->prepare("UPDATE $oqey_music SET status = '%d' WHERE id = '%d' ", 0, $_POST['restoresong'] ));
...
function oQeySongRename(){
   ...
   $m = $wpdb->query( $wpdb->prepare("UPDATE $oqey_music SET title = '%s' WHERE id = '%d' ",$title, $id));
   echo stripslashes_deep($title);
   die();
}
/*END music rename*/
...
function oQeySkinPermDelete(){
   ...
   $u = $wpdb->query( $wpdb->prepare( "UPDATE $oqey_galls SET skin_id = '%d' WHERE skin_id = '%d' ", 0, $_POST['deleteskinper'] ) );
