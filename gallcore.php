...
function addoQeyMediaIcon($context){
    ...
    //return "<a href='" . esc_url( get_upload_iframe_src($type) ) . "'  class='thickbox' title='$title'><img src='" . esc_url( admin_url( $icon ) ) . "' alt='$title' onclick='return false;' /></a>";
}
...
function oqey_add_pages() {
    ...
    add_submenu_page($oqeym, 'Video', 'Video', 'oQeyVideo', 'oQeyVideo', 'oqey_video_page');    
   ...
   if($gal){
      ...
      if ($isCrawler || (is_plugin_active('wptouch/wptouch.php') && IBROWSER)){
        ...
        foreach($all as $i){ 
          ...
          if($i->img_type=="nextgen"){
           ...
           if($i->img_type!="video"){
             ...
             $imgs .= '<img src="'.$gimg.trim($i->title).'" alt="Photo '.urlencode(trim($i->alt)).'" width="'.$oqey_width.'" style="margin-top:3px;"/>'; 
        ...
        if(get_option("oqey_backlinks")=="on"){ 
           ...
           $imgs .= '<div style="font-size:11px;margin-left:auto;margin-right:auto;width:100%;text-align:center;font-family:Arial">powered by <a href="http://oqeysites.com" target="_blank">oQeySites</a></div>';  
        }
	...
	foreach($all as $i){ 
       ...
       if( OQEYBROWSER && $i->img_type=="video" && ($vpath["extension"]=="mp4" || $vpath["extension"]=="m4p") ){
            ...
            if( !empty($vdurl['host']) ){
                ...
                $vurl = trim($i->title);
                //$imgs .= '[div class="oqeyimgdiv" style="width:'.$oqey_width.'px; height:'.($oqey_width/1.5).'px; margin-top:3px;"][video id="videocnt" width="'.$oqey_width.'" height="'.($oqey_width/1.5).'" controls][source src="'.$vurl.'"  type="video/mp4"][/video][/div]';
        
            }else{
        
                $vurl = get_option('siteurl').'/'.trim($i->title);
               
              // if( is_file(OQEY_ABSPATH.trim($i->title) )){ 
	...
	if ( $size ){
       ...
       //$imgs .= '[div class="oqeyimgdiv" style="background: url('.$img_full_root.') center top no-repeat;border:blue thin solid; width:'.$oqey_width.'px; height:'.$img_holder_h.'px;'.$top_margin.'"]'.$customlink.'[/div]';
       $imgs .= '[div class="oqeyimgdiv" style="background: url('.$img_full_root.') center top no-repeat;width:'.$oqey_width_n.'px;height:'.$img_holder_h.'px;'.$top_margin.'"]'.$customlink.'[/div]';
         //$imgs .= '[div class="oqeyimgdiv" style="background: url('.$img_full_root.') center center no-repeat; width:'.$oqey_width.'px; height:'.$img_holder_h.'px;'.$top_margin.'"][/div]';
    ...
    //<button style="background: url({$plugin_url_qu}/images/arrow-left.png) center center no-repeat; width:53px; height:101px; border:none;">&nbsp;</button>
    //<button style="background: url({$plugin_url_qu}/images/arrow-right.png) center center no-repeat; width:53px; height:101px; border:none;">&nbsp;</button>
   //<div id="image{$oqeycounter}" style="height:auto; display:none;" class="oqey_images"></div>
...
ob_start();	
print <<< SWF
<div id="oqey_image_div{$oqeycounter}" style="position:relative;width:{$oqey_width_n}px;height:{$custom_height_n}px;display:none;margin: 0 auto;{$div_custom_margin}{$custom_bg_img}">
...
<div id="image{$oqeycounter}" style="width:{$c_width}px; height:{$img_holder_h}px;display:none;background:transparent;margin: 0 auto;{$custom_margin_top}" class="oqey_images"></div>
