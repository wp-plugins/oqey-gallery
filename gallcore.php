...
//...............................................//////////////////////////////////////
add_action('save_post', 'oqey_gallery_in_post'); // process curent save post
...
$control = checkPost($post_id); // get current post type
if($control == "page" || $control == "post"){// check if sended Post_id is an post or page type
...
$post_id_revision = $arr[0];

$revision_content = $wpdb->get_var("SELECT post_content
							          FROM $wpdb->posts
							         WHERE ID = '$post_id_revision' 
							           AND post_type = 'revision'
							           AND post_parent = '$post_id'");

$zero = 0;
$zero_update = sprintf("UPDATE %s SET post_id=%d WHERE post_id = %d", $oqey_galls, $zero, $post_id);
$update_zero = mysql_query($zero_update) or die (mysql_error());
$content = preg_replace_callback( "/\[oqeygallery:([^]]+)\s+([^]]+)]/i", "add_post_id_to_db", $content );

}
}

function add_post_id_to_db($gasite){
global $wpdb;
$oqey_galls = $wpdb->prefix . "oqey_gallery";
$oqey_images = $wpdb->prefix . "oqey_images";
$galleryID = $gasite[1];
...
$gal_update = sprintf("UPDATE %s SET post_id=%d WHERE id = %d", $oqey_galls, $post_i_id, $galleryID);
$update_gal = mysql_query($gal_update) or die (mysql_error());
...
if (is_feed()) {
$content = preg_replace_callback( "/\[show_all_qgalleries_in_page]/i", "add_oqey_galleries_feed", $content );
}
return $content;
}
...
if(numberRegex.test(x)) {
if(parseFloat(x)<3){ jQuery("#oqey_pause_between_tran").val("3"); }
...
<tr>
    ...
    <td width="542"><input name="oqey_pause_between_tran" id="oqey_pause_between_tran" type="text" value="<?php echo get_option('oqey_pause_between_tran'); ?>" size="5"/>
      seconds</td>
</tr>
</table>
<p>&nbsp;</p>
<p>
  ...
  <input type="hidden" name="page_options" value="oqey_height,oqey_width,oqey_bgcolor,oqey_thumb_width,oqey_thumb_height,oqey_max_width,oqey_max_height,limitmax,crop_images,oqey_effects_trans_time,oqey_pause_between_tran" />
</p>
<p class="submit">
...
if (is_feed()) {
$content = preg_replace_callback( "/\[oqeygallery:([^]]+)\s+([^]]+)]/i", "add_oqey_gallery_feed", $content );
}else{
$content = preg_replace_callback( "/\[oqeygallery:([^]]+)\s+([^]]+)]/i", "add_gallery", $content );
}
return $content;
}
...
function add_gallery($gasite){// insert flash gallery in content
