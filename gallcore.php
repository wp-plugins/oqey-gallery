...
class oQey_Gallery_Widget extends WP_Widget {
   ...
   /*images array*/     
    ...
    if($incolums!="on"){
      ...
      $nobject .='data-cycle-loader="wait"
                  '.$autostart.'
                  '.$optouch.'
                  '.$arrowleftright.'
   ...
   foreach($all as $i){ 
	...
	$nobject .= '<div><img src="'.$imglink.'" title="'.trim($i->comments).'"/></div>'."\n";
...
if($incolums!="on"){
  ...
  $object .= 'jQuery(".cycle-slideshow'.$oqeycounter.'").css("max-height", '.$oqey_height.');'."\n";
  $object .= 'jQuery(".cycle-slideshow'.$oqeycounter.' div img").css("max-height", '.$oqey_height.');';
...
?>
	<p>
        ...
        <p>
            <label for="<?=$this->get_field_id('width')?>"><?php _e('Width:', 'oqey-gallery'); ?></label> 
            <input id="<?=$this->get_field_id('width'); ?>" name="<?=$this->get_field_name('width')?>" type="text" style="padding:3px;width:45px;" value="<?=$width?>" /> px
        </p>
        <p>
            <label for="<?=$this->get_field_id('height')?>"><?php _e('Height:', 'oqey-gallery'); ?></label>
            <input id="<?=$this->get_field_id('height')?>" name="<?=$this->get_field_name('height')?>" type="text" style="padding:3px;width:45px;" value="<?=$height?>" /> px
        </p>
        <p>
            <label for="<?=$this->get_field_id('autostart')?>"></label>
            <input id="<?=$this->get_field_id('autostart')?>" name="<?=$this->get_field_name('autostart')?>" type="checkbox" value="1" <?php checked(true , $autostart); ?> />
                <?php _e('Autostart','oqeygallery'); ?>
        </p>
        <p>
            <label for="<?=$this->get_field_id('oqeywidgettext')?>"><?php _e('Few words:', 'oqey-gallery'); ?></label>
            <textarea id="<?=$this->get_field_id('oqeywidgettext')?>" name="<?=$this->get_field_name('oqeywidgettext')?>" style="width:100%;"><?=trim($oqeywidgettext)?></textarea>
        </p>		
        <?php }
}
/*End Widget*/
...
function oqey_gallery_front_head(){
   ...
   $cyclejs = WP_PLUGIN_URL . '/oqey-gallery/js/jquery.cycle2.min.js';
   echo "\n".'<script type="text/javascript" src="' . $cyclejs . '"></script>';
      
   $cycleswjs = WP_PLUGIN_URL . '/oqey-gallery/js/jquery.cycle2.swipe.js';
   echo "\n".'<script type="text/javascript" src="'. $cycleswjs . '"></script>';
      
   $cyclectjs = WP_PLUGIN_URL.'/oqey-gallery/js/jquery.cycle2.center.min.js';
   echo "\n".'<script type="text/javascript" src="'.$cyclectjs.'"></script>';
              
   /*$iosfix = WP_PLUGIN_URL . '/oqey-gallery/js/ios6fix.js';
   echo "\n".'<script type="text/javascript" src="' . $iosfix . '"></script>';*/
...
function AddoQeyGallery($atts){
	...
	foreach($all as $i){ 
              ...
              if($i->img_type!="video" ){
                  ...
                  if($i->img_type=="nextgen"){
                      ...
                      $nobject .= '<div><img src="'.$imglink.'" title="'.trim($i->comments).'" style="max-width:100%;"/></div>'."\n";
   ...
   wp_enqueue_script('oqeyjs', WP_PLUGIN_URL . '/oqey-gallery/js/oqey.js', array('jquery'),'', false);
