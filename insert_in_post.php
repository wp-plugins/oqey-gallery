...
if (!empty($_SERVER['SCRIPT_FILENAME']) && 'insert_in_post.php' == basename($_SERVER['SCRIPT_FILENAME']))
   ...
   $img_root = get_option('siteurl').'/wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid);
...
<?php 
   ...
   if(is_file($img_path.$i->folder.'/galthmb/'.$img->title) || $img->status!=2){
   }else{
	$img = $wpdb->get_row("SELECT * FROM $oqey_images WHERE gal_id ='".$i->id."' AND status!=2 ORDER BY img_order LIMIT 0,1");
   }
   }else{
    $img = $wpdb->get_row("SELECT * FROM $oqey_images WHERE gal_id ='".$i->id."' AND status!=2 ORDER BY img_order LIMIT 0,1");
   }
   if($j%2){ $colorb = '#F4FAFF'; }else{ $colorb = '#E8F4FF'; }  
	...
	echo '<tr>	
			  ...
			  <img src="'.$img_root.$i->folder.'/galthmb/'.$img->title.'" alt="image" class="addinpost" style="border:0; max-width:120px; max-height:80px;"/></a>
              </div>	           
