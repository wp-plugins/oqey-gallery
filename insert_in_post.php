           ...
           if(!empty($images)){
               ...
               if($i->splash_img!=0){
                ...
                if($img->img_type=="nextgen"){
                    ...
                    $gthmbnew = get_option('siteurl').'/wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).'/'.trim($i->folder).'/galthmb/';
                                        
                } 
                
                echo '<tr><td style="background-color:'.$colorb.';">
                       <div>		
                        <div style="float:left; width:120;">
                        <a href="#null" onclick="inoQeyContent(\''.$i->id.'\')">
                            <img src="'.$gthmbnew.$img->title.'" alt="image" class="addinpost" style="border:0; max-width:120px; max-height:80px;"/>
                        </a>
                        </div>
                        <div align="left" style="float:left; padding-left:15px;">'.trim($i->title).'</div>
                        </div>
                        </td>
                       </tr>';
		  $j++;
