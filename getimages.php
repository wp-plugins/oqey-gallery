...
$id = (int)mysql_real_escape_string($_REQUEST['gal_id']);
...
foreach($imgs as $i) { 
$r .= '<item>
<thumb file="'.trim($i->title).'" alt="'.urlencode(trim($i->alt)).'" comments="'.urlencode(trim($i->comments)).'" />
<image file="'.trim($i->title).'" alt="'.urlencode(trim($i->alt)).'" comments="'.urlencode(trim($i->comments)).'" />
</item>
';
}
$r .= '</oqeygallery>';
