...
function custom_shortcode_scripts() {
	...
	//if( has_shortcode( $post->post_content, 'oqeygallery') ) {
            wp_register_script('cycle2', WP_PLUGIN_URL . '/oqey-gallery/js/jquery.cycle2.min.js', array('jquery'),'', true);
            wp_register_script('cycle2.swipe', WP_PLUGIN_URL . '/oqey-gallery/js/jquery.cycle2.swipe.min.js', array('cycle2'),'', true);
            wp_register_script('oqeyjs', WP_PLUGIN_URL . '/oqey-gallery/js/oqey.js', array('jquery'),'', true);
	//}
}

add_action( 'wp_enqueue_scripts', 'custom_shortcode_scripts');

function oQeyAddJavaScriptLibraries(){
...
function oqey_gallery_in_post($post_id) {
   ...
   if($control == "page" || $control == "post"){
       ...
       $content = $wpdb->get_var( $wpdb->prepare("SELECT post_content FROM $wpdb->posts WHERE ID = %d AND post_type != 'revision'", $post_id ) );
       $wpdb->query( $wpdb->prepare( "UPDATE {$oqey_galls} SET post_id = %s WHERE post_id = %s", 0, $post_id ) );
       preg_match_all('/\[oqeygallery id=([^]]+)]/i', $content, $gals);
       
       foreach($gals[1] as $id){ 
           
           $wpdb->query( $wpdb->prepare("UPDATE $oqey_galls SET post_id = %d WHERE id = %d ", $post_id, $id ) );
