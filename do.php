...
if($add){
   $img = OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galimg";
   $thumb = OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galthmb";   
   $iphone = OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/iphone";   
   wp_mkdir_p($img);
   wp_mkdir_p($thumb);
   wp_mkdir_p($iphone);
...
$aid = $wpdb->get_row("SELECT author FROM $oqey_galls WHERE id ='".esc_sql($_POST['newgallid'])."'");
...
$resp["response"] .= '<table width="900" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="133" height="50" align="left" valign="middle">
	  ...
	  var flashvars = {BatchUploadPath:"'.OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.$gal->folder.'",
...
$f = OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.$gal->folder;
if(!is_dir($f)){
...
if($d){
 if(is_file(OQEY_ABSPATH."wp-content/oqey_gallery/music/".$_POST['name'])){    
	$i = unlink(OQEY_ABSPATH."wp-content/oqey_gallery/music/".$_POST['name']);
   }
    if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_music WHERE status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
//$all = explode(",", urldecode($_POST['selectedmusic']));
...
if($d){
 if(is_dir(OQEY_ABSPATH."wp-content/oqey_gallery/skins/".esc_sql($_POST['name'])) ){   
 $dir = OQEY_ABSPATH."wp-content/oqey_gallery/skins/".esc_sql($_POST['name']);
 $do = rm($dir);
if($do){ 
if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_skins WHERE status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
if($d){
 if(is_dir(OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".esc_sql($_POST['name'])) ){   
 $dir = OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".esc_sql($_POST['name']);
 $do = rm($dir);
if($do){ 
if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_galls WHERE status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
global $wpdb;
$oqey_galls = $wpdb->prefix . "oqey_gallery";	
$r = $wpdb->get_row("SELECT folder FROM $oqey_galls WHERE id ='".esc_sql($id)."'");
return $r->folder;
}
...
$d = $wpdb->query("DELETE FROM $oqey_images WHERE gal_id = '".esc_sql($galid)."' AND title = '".esc_sql($title)."' ");
 ...
 if(is_file(OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galimg/".$title)){    
	$i = unlink(OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galimg/".$title);
	$t = unlink(OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galthmb/".$title);
	$t = unlink(OQEY_ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/iphone/".$title);
   }					
}
