...
if($add){
   $img = ABSPATH."/wp-content/oqey_gallery/galleries/".$folder."/galimg";
   $thumb = ABSPATH."/wp-content/oqey_gallery/galleries/".$folder."/galthmb";   
   $iphone = ABSPATH."/wp-content/oqey_gallery/galleries/".$folder."/iphone";   
   wp_mkdir_p($img);
   wp_mkdir_p($thumb);
   wp_mkdir_p($iphone);
...
$resp["response"] .= '<table width="900" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="133" height="50" align="left" valign="middle">
	  ...
	  var flashvars = {BatchUploadPath:"'.ABSPATH.'wp-content/oqey_gallery/galleries/'.$gal->folder.'",
...
$f = ABSPATH.'wp-content/oqey_gallery/galleries/'.$gal->folder;
if(!is_dir($f)){
...
if($d){
 if(is_file(ABSPATH."wp-content/oqey_gallery/music/".$_POST['name'])){    
	$i = unlink(ABSPATH."wp-content/oqey_gallery/music/".$_POST['name']);
   }
    if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_music WHERE status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
<param name="movie" value="'.$plugin_url_qu.'/skins/'.$skin->folder.'/'.$skin->folder.'.swf">
<param name="FlashVars" value="flashId='.$skin->skinid.'&amp;FKey='.$skin->comkey.'&amp;GalleryPath='.$plugin_url_qu.'&amp;GalleryID='.$galleryID.'&amp;FirstRun='.$skin->firstrun.'">
<embed src="'.$plugin_url_qu.'/skins/'.$skin->folder.'/'.$skin->folder.'.swf" bgcolor="#ffffff" FlashVars="flashId='.$skin->skinid.'&amp;FKey='.$skin->comkey.'&amp;GalleryPath='.$plugin_url_qu.'&amp;GalleryID='.$galleryID.'&amp;FirstRun='.$skin->firstrun.'" width="896" height="600" wmode="transparent" allowFullScreen="true"></embed>
</object>
';
}
}
...
if(!empty($get_list)){
	...
	if($r->commercial=="yes"){ $comm = " - Commercial skin"; }else{ $comm = " - Free skin"; }
       ...
       echo '<tr id="skink_tr_'.$r->id.'">
			 ...
			 <img src="'.oQeyPluginUrl().'/skins/'.$r->folder.'/'.$r->folder.'.jpg" alt="skin" width="150" height="100" style="border:#999999 solid thin; margin-left:15px;" >
			 </td>
...
if($d){
 if(is_dir(ABSPATH."wp-content/plugins/oqey-gallery/skins/".mysql_real_escape_string($_POST['name'])) ){   
 $dir = ABSPATH."wp-content/plugins/oqey-gallery/skins/".mysql_real_escape_string($_POST['name']);
 $do = rm($dir);
if($do){ 
if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_skins status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
echo '<table width="900" border="0" cellspacing="0" cellpadding="0" id="currentskintable">
	  ...
	  <tr id="skink_tr_'.$r->id.'">
             <td width="180" height="120" align="center" valign="middle">
			   <div align="center"><img src="'.oQeyPluginUrl().'/skins/'.$r->folder.'/'.$r->folder.'.jpg" alt="skin" width="150" height="100" style="border:#999999 solid thin;"/>
	           </div></td>
...
if($d){
 if(is_dir(ABSPATH."wp-content/oqey_gallery/galleries/".mysql_real_escape_string($_POST['name'])) ){   
 $dir = ABSPATH."wp-content/oqey_gallery/galleries/".mysql_real_escape_string($_POST['name']);
 $do = rm($dir);
if($do){ 
if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_galls WHERE status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
$d = $wpdb->query("DELETE FROM $oqey_images WHERE gal_id = '".mysql_real_escape_string($_POST['galid'])."' AND title = '".mysql_real_escape_string($title)."' ");
 ...
 if(is_file(ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galimg/".$title)){    
	$i = unlink(ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galimg/".$title);
	$t = unlink(ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/galthmb/".$title);
	$t = unlink(ABSPATH."wp-content/oqey_gallery/galleries/".$folder."/iphone/".$title);
   }					
}
