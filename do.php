...
$newtitle = esc_sql(stripslashes_deep(trim(urldecode($_POST["gal_edit_title"]))));
...
$gal_update = sprintf("UPDATE $oqey_galls SET title = '%s' WHERE id = '%d' ",$title, $gal_id);						
...
$images = $wpdb->get_results("SELECT * FROM $oqey_images WHERE gal_id ='".esc_sql($_POST['allimages'])."' AND status !=2 ORDER BY img_order ASC, id DESC");
$folder = oqey_get_gallery_folder($_POST['allimages'])."/galthmb/";
$imgFolder = oqey_get_gallery_folder($_POST['allimages'])."/galimg/";

$s = $wpdb->get_row("SELECT splash_img FROM $oqey_galls WHERE id ='".esc_sql($_POST['allimages'])."'");
...
$song_trash = sprintf("UPDATE $oqey_music SET status = '%d' WHERE id = '%d' ", "2", esc_sql($_POST['songtotrash']) );						
...
$song_trash = sprintf("UPDATE $oqey_music SET status = '%d' WHERE id = '%d' ", "0", esc_sql($_POST['restoresong']) );						
$song = mysql_query($song_trash) or die (mysql_error());
if($song){
...
$d = $wpdb->query("DELETE FROM $oqey_music WHERE id = '".esc_sql($_POST['deletesong'])."' ");
$r = $wpdb->query("DELETE FROM $oqey_music_rel WHERE music_id = '".esc_sql($_POST['deletesong'])."' ");

if($d){
 if(is_file(OQEY_ABSPATH."wp-content/oqey_gallery/music/".oqey_getBlogFolder($wpdb->blogid).$_POST['name'])){    
	$i = unlink(OQEY_ABSPATH."wp-content/oqey_gallery/music/".oqey_getBlogFolder($wpdb->blogid).$_POST['name']);
   }
    if(!$sql=$wpdb->get_row($wpdb->prepare("SELECT id FROM $oqey_music WHERE status = '%d' ", "2" ))){ $e="no"; }
echo $e;
...
$gal_trash = sprintf("UPDATE $oqey_galls SET status = '%d' WHERE id = '%d' ", "2", esc_sql($_POST['movetotrashgall']) );						
...
$resp = array();
$gal_trash = sprintf("UPDATE $oqey_galls SET status = '%d' WHERE id = '%d' ", "0", esc_sql($_POST['undogallid']) );						
$gal = mysql_query($gal_trash) or die (mysql_error());
...
$img_trash = sprintf("UPDATE $oqey_images SET status = '%d' WHERE id = '%d' ", "2", esc_sql($_POST['delimgid']) );						
...
$img_trash = sprintf("UPDATE $oqey_images SET status = '%d' WHERE id = '%d' ", "0", esc_sql($_POST['undoimgid']) );						
$img = mysql_query($img_trash) or die (mysql_error());
if($img){
...
foreach ($all as $id){
$img_trash = sprintf("UPDATE $oqey_images SET status = '%d' WHERE id = %d ", "2", esc_sql($id));						
$img = mysql_query($img_trash) or die (mysql_error());
$i++;
}
if($i>0){
...
$i=0;
foreach ($all as $id){
$img_trash = sprintf("UPDATE $oqey_images SET status = '%d' WHERE id = %d ", "0", esc_sql($id));						
$img = mysql_query($img_trash) or die (mysql_error());
$i++;
}
if($i>0){
...
$up = sprintf("UPDATE $oqey_galls SET author = '%d' WHERE id = '%d' ", esc_sql($_POST['usrid']), esc_sql($_POST['gid']) );						
...
$ruta_img_bg = get_option('siteurl').'/wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/';
$thumb_root = get_option('siteurl').'/wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galthmb/';
...
$show = "yes";
  	 $get_bg_img = $wpdb->get_row("SELECT *
								     FROM $oqey_galls 
							        WHERE id ='".esc_sql($galleryID)."'
                                  ");			  

$extract_images = $wpdb->get_results("SELECT *
                                        FROM $oqey_images
                                       WHERE gal_id = '".esc_sql($galleryID)."'
...
$outputIMG .= '</div>';		  
      if($get_bg_img->splash_img !=0){
	 	$get_bg_img2 = $wpdb->get_row("SELECT *
                                        FROM $oqey_images
							           WHERE id ='".$get_bg_img->oqey_thumb_img."'  ");
...
$up = sprintf("UPDATE $oqey_images
					        ...
					        WHERE id = '%d'
					      ", esc_sql(stripslashes_deep(urldecode($_POST['alt']))),
						     esc_sql(stripslashes_deep(urldecode($_POST['comments']))),
					         esc_sql($_POST['imgid'])
				          );	
$alt_up = mysql_query($up) or die (mysql_error()); // image_alt

$m = "not";
$c = $wpdb->get_row("SELECT splash_img FROM $oqey_galls WHERE id ='".esc_sql($_POST['galid'])."'");

if($c->splash_img==$_POST['imgid']){
$splash = sprintf("UPDATE $oqey_galls SET splash_img ='%d',splash_only = '%d' WHERE id = '%d'
			      ", 0, 0, esc_sql($_POST['galid'])  );	
...
$splash = sprintf("UPDATE $oqey_galls SET splash_img ='%d',splash_only = '%d' WHERE id = '%d'
			      ", esc_sql($_POST['imgid']), 1, esc_sql($_POST['galid']) );						
$splashk = mysql_query($splash) or die (mysql_error());
$m = $_POST['imgid'];
}elseif(isset($_POST['splash'])){	
	
$splash = sprintf("UPDATE $oqey_galls SET splash_img ='%d',splash_only = '%d' WHERE id = '%d'
			      ", esc_sql($_POST['imgid']), 0, esc_sql($_POST['galid']) );						
$splashk = mysql_query($splash) or die (mysql_error());
$m = $_POST['imgid'];
}
echo $m;
...
$i=0;
if(count($_POST['selectedmusic'])>0){
...
$u = sprintf("UPDATE $oqey_skins SET comkey = '%s' WHERE id = '%d' ", esc_sql($_POST['comkey']), esc_sql($_POST['savekey']) );						
...
$skin = sprintf("UPDATE $oqey_skins SET status = '%d' WHERE id = '%d' ", "2", esc_sql($_POST['movetotrashskin']) );						
...
$resp = array();
$skin = sprintf("UPDATE $oqey_skins SET status = '%d' WHERE id = '%d' ", "0", esc_sql($_POST['undoskin']) );						
$s = mysql_query($skin) or die (mysql_error());
if($s){
...
$d = $wpdb->query("DELETE FROM $oqey_skins WHERE id = '".esc_sql($_POST['deleteskinper'])."' ");
$u = sprintf("UPDATE $oqey_galls SET skin_id = '%d' WHERE skin_id = '%d' ", 0, esc_sql($_POST['deleteskinper']));						
$upd= mysql_query($u) or die (mysql_error());
...
$skin = sprintf("UPDATE $oqey_galls SET skin_id = '%d' WHERE id = '%d' ", esc_sql($_POST['skinid']), esc_sql($_POST['skin_gallery_id']) );						
$s = mysql_query($skin) or die (mysql_error());
if($s){
...
$e="";
$d = $wpdb->query("DELETE FROM $oqey_galls WHERE id = '".esc_sql($_POST['deletegall'])."' ");
$r = $wpdb->query("DELETE FROM $oqey_music_rel WHERE gallery_id = '".esc_sql($_POST['deletegall'])."' ");
...
$d = $wpdb->query("DELETE FROM $oqey_images WHERE gal_id = '".esc_sql($galid)."' AND title = '".esc_sql($title)."' ");
 ...
 if(is_file(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.$title)){    
	$i = unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.$title);
	$t = unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galthmb/'.$title);
	$t = unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.$title);
   }					
}
...
/*get new skins from oqeysites.com*/
if(isset($_POST['get_new_skins'])){
    ...
    if(count($xml)>0){
		/*
                    [title] =&gt; oQey+Default+Skin
                    [description] =&gt; That%60s+the+default+grey+skin+that+comes+with+the+free+oQey+Galleries+pack
                    [commercial] =&gt; no
                    [skinid] =&gt; 0001
                    [price] =&gt; 22
                    [discountcode] =&gt; 0
                    [discount] =&gt; 0
                    [archivename] =&gt; oqeymainsk
                    [comkey] =&gt; 0
      */
       ...
       //if($x->discountcode!="" && $x->commercial=="yes"){ $discount_code = 'Discount code: '.urldecode($x->discountcode).' - Discount of <span style="color:red;">'.$x->discount.'</span>% <br/>'; }else{ $discount_code = "<br/>"; }
	...
	echo "There is no new skins.";	
	}
}


if(isset($_POST['install_new_skin'])){
...
if(isset($_POST['get_the_preview'])){
