...
foreach ($galls as $i){
	...
	$imgs = $wpdb->get_results("SELECT * FROM $oqey_images WHERE gal_id = '".$i->gal_id."' AND status=2");
    $folder = $i->folder;

foreach ($imgs as $img){
$d = $wpdb->query("DELETE FROM $oqey_images WHERE id = '".$img->id."'");

 if(is_file(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.$img->title)){    
	$i = unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.$img->title);
	$t = unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galthmb/'.$img->title);
	$t = unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.$img->title);
   }
  }	
 }
/*end*/
...
if($d){
 if(is_dir(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$g->folder)){   
 $dir = OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$g->folder;
 $do = oqey_rm($dir);
  } 
 }
}
...
$u = $wpdb->query( $wpdb->prepare("UPDATE {$oqey_galls} SET skin_id = '0' WHERE skin_id='".$s->id."'"));

if($d){
 if(is_dir(OQEY_ABSPATH."wp-content/oqey_gallery/skins/".oqey_getBlogFolder($wpdb->blogid).$s->folder)){   
 $dir = OQEY_ABSPATH."wp-content/oqey_gallery/skins/".oqey_getBlogFolder($wpdb->blogid).$s->folder;
 $do = oqey_rm($dir);
  }
 }
}
...
if($d){
 if(is_file(OQEY_ABSPATH."wp-content/oqey_gallery/music/".oqey_getBlogFolder($wpdb->blogid).$m->link)){    
	$i = unlink(OQEY_ABSPATH."wp-content/oqey_gallery/music/".oqey_getBlogFolder($wpdb->blogid).$m->link);
  }
 }
}
/*end*/
...
?>
<div class="wrap">
        <h2>Trash <?php if(!empty($get_music) || !empty($get_skins) || !empty($get_g_list) || !empty($galls)){ echo "| <a href='#empty_trash' id='empty_trash' class='empty_trash'>Empty Trash</a>"; } ?></h2>
<div id="save" style="width:953px; margin-bottom:10px; margin-left:-5px;"><?php echo $mesaj; ?></div>
</div> 
...
$r .= '<li id="img_li_'.$img->id.'"'.$b.'><div class="allbut" align="center">
		               ...
		               <img src="'.get_option('siteurl').'/wp-content/plugins/oqey-gallery/images/separator_line.png" width="120" height="2" align="middle"/>
...
if(id==2){
		...
		jQuery.post("<?php echo oQeyPluginUrl(); ?>/do.php", { imgallfromtrash: encodeURIComponent(names), galid: galid },
        function(data){
		if(names.length>0){
...
if(id==3){
        ...
        jQuery.post("<?php echo oQeyPluginUrl(); ?>/do.php", { imgalldelete: encodeURIComponent(names), galid: galid },
        function(data){
		if(names.length>0){			
...
jQuery("#empty_trash").click(function(){
		...
		window.location = "<?php echo get_option('siteurl'); ?>/wp-admin/admin.php?page=oQeyTrash&empty_trash=yes&wpnonce=<?php echo wp_create_nonce('oqey_empty_trash'); ?>"		
} 
});
