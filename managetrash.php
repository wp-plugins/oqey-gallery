...
if ( !current_user_can('oQeyTrash') ) die(__('You do not have sufficient permissions for this page.'));
    ...
    $d = $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_images WHERE id = %d ", $img->id ) );
     ...
     if($img->img_type=="nextgen"){
        ...
        unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.trim($img->title));
        unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galthmb/'.trim($img->title));
        
        if(is_file(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.trim($img->title))){ 
           unlink(OQEY_ABSPATH.'wp-content/oqey_gallery/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.trim($img->title));
        }
...
?>
  <tr style="padding:3px;" id="row_<?php echo $music->id; ?>">
    ...
    <div class="click" id="select_<?=$music->id?>"><?=$music->title?></div>    </td>
    <td width="27" align="right" valign="middle" class="lasttd">     
      <a href="#null" onclick="restoreSong('<?=$music->id?>'); return false;" class="hiddenm">
    ...
    <td width="27" align="left" valign="middle" class="lasttd">
      <a href="#null" onclick="deleteSong('<?=$music->id?>', '<?=$music->link?>'); return false;" class="hiddenm">
...
?>
          <tr id="video_tr_<?php echo $video->id; ?>">
             ...
             <td width="630" align="left" valign="top" style="margin-left:10px; padding:5px;">
                 ...
                 <p><a href="#restore" class="restorevideo" onclick="restoreVideo('<?=$video->id?>'); return false;"><?php _e('Restore', 'oqey-gallery'); ?></a> | 
                     <a href="#delete_this_video" class="delete_this_video" onclick="deleteVideo('<?=$video->id?>', '<?=$video->video_link?>'); return false;"><?php _e('Delete Permanently', 'oqey-gallery'); ?></a></p>
             </td>
           </tr>
<?php } 
    ...
    jQuery.post(ajaxurl, { action:"oQeyImagesFromTrash", imgallfromtrash: encodeURIComponent(names), galid: galid },
        function(data){
           ...
           if(names.length>0){
              ...
              x.find(".messages").hide().html('<?=$beforemsg?><?php _e('&nbsp;All selected images was restored.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow");
              var data = eval('(' + data + ')');
		...
		x.find(".messages").hide().html('<?=$beforemsg?><?php _e('&nbsp;Please select an image', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow"); 
		clearUp(); 
    ...
    if(id==3){
        ...
        function(data){
          if(names.length>0){	
            ...
            x.find(".messages").hide().html('<?=$beforemsg?><?php _e('&nbsp;All selected images was deleted permanently.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow");
...
jQuery.post(ajaxurl, { action:"oQeyFromTrashVideo", id: id },
   function(data){
			...
			jQuery("#save").hide().html('<?=$beforemsg?>' + data.mesaj + '<?=$aftermsg?>').fadeIn("slow"); 
   });
}
...
function deleteVideo(id, name){ 
   ...
   function(data){
            ...
            jQuery("#save").hide().html('<?=$beforemsg?><?php _e('&nbsp;Video was deleted.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow"); 
   });
...
function deleteSong(id, name){ 
   ...
   function(data){
            ...
            jQuery("#save").hide().html('<?=$beforemsg?><?php _e('&nbsp;Song was deleted.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow"); 
   });
...
jQuery.post(ajaxurl, { action:"oQeySongFromTrash", restoresong: id },
   function(data){
            jQuery("#row_" + id).fadeOut('slow');
			if(data=="no"){ jQuery("#tabs-11").fadeOut(); }
			jQuery("#save").hide().html('<?=$beforemsg?><?php _e('&nbsp;Song was restored.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow"); 
   });
}
...
jQuery.post(ajaxurl, { action:"oQeySkinPermDelete", deleteskinper: id, name: name },
   function(data){
			...
			if(data=="no"){ jQuery("#tabs-22").fadeOut(); }
                        jQuery("#save").hide().html('<?=$beforemsg?><?php _e('Skin was deleted.', 'oqey-gallery'); ?><?=$aftermsg?><\/div><\/div>').fadeIn('slow');
...
jQuery.post(ajaxurl, { action:"oQeyGalleryFromTrash", undogallid: id },
   function(data){
			...
			jQuery("#save").hide().html('<?=$beforemsg?><?php _e('Gallery was restored.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow"); 
...
jQuery.post(ajaxurl, { action:"oQeyGalleryPermDelete", deletegall: id, name: name },
   function(data){
			...
			jQuery("#save").hide().html('<?=$beforemsg?><?php _e('Deleted.', 'oqey-gallery'); ?><?=$aftermsg?>').fadeIn("slow"); 
   });
}
