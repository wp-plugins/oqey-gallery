...
global $wpdb;
   ...
   $mesaj          = "";
   $beforemsg      = '<div class="wrap"><div class="updated below-h2" id="message"><p>';
   $aftermsg       = '</p></div></div>';
...
if ( !current_user_can('oQeyTrash') ) die(__('You do not have sufficient permissions for this page.'));
    ...
    $d = $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_images WHERE id = %d ", $img->id ) );

     if($img->img_type=="oqey"){
        
        $folder = oqey_get_gallery_folder($img->gal_id);         
       	
        unlink(oQeyPluginRepoPath().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galimg/'.trim($img->title));
        unlink(oQeyPluginRepoPath().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/galthmb/'.trim($img->title));
        
        if(is_file(oQeyPluginRepoPath().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.trim($img->title))){ 
           unlink(oQeyPluginRepoPath().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).$folder.'/iphone/'.trim($img->title));
        }        
     } 
}	
...
foreach ($get_g_list as $g){
  ...
  $d = $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_galls WHERE id = %d ", $g->id ) );
  
  if($d){
   
   $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_images WHERE gal_id = %d ", $g->id ) );
   $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_music_rel WHERE gallery_id = %d ", $g->id ) );
  
   if(is_dir(oQeyPluginRepoPath().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).trim($g->folder))){   
     
     $dir = oQeyPluginRepoPath().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).trim($g->folder);
     $do  = oqey_rm($dir);
   } 
  }
}
...
$get_skins = $wpdb->get_results("SELECT * FROM $oqey_skins WHERE status=2");
   ...
   foreach ($get_skins as $s){
     ...
     $d      = $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_skins WHERE id = %d ", $s->id ));
     
   if($d){
     
     $wpdb->query( $wpdb->prepare("UPDATE $oqey_galls SET skin_id = '0' WHERE skin_id = %d ", $s->id));
     
     $skopt = "oqey_request_key_".$skinid;
     delete_option($skopt);
    
     if(is_dir(oQeyPluginRepoPath().'/skins/'.oqey_getBlogFolder($wpdb->blogid).trim($s->folder))){   
     
        $dir = oQeyPluginRepoPath().'/skins/'.oqey_getBlogFolder($wpdb->blogid).trim($s->folder);
        $do  = oqey_rm($dir);
...
$get_music = $wpdb->get_results("SELECT * FROM $oqey_music WHERE status = 2 ");
    ...
    foreach ($get_music as $m){	
       ...
       $d = $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_music WHERE id = %d ", $m->id ));
       $r = $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_music_rel WHERE music_id = %d ", $m->id) );
...
/*Delete all videos*/
  ...
  foreach($videos as $v){
        ...
        $wpdb->query( $wpdb->prepare("DELETE FROM $oqey_images WHERE video_id = %d ", $v->id ));
        
        /*if($v->type !="external"){
           
            if(is_file(OQEY_ABSPATH.trim($v->video_link))){  
                unlink(OQEY_ABSPATH.trim($v->video_link));
            }
  ...
  $galls          = $wpdb->get_results("SELECT COUNT($oqey_images.id) as numartotal, $oqey_images.gal_id,  $oqey_galls.title, $oqey_galls.folder, $oqey_galls.splash_img
                                          FROM $oqey_images, $oqey_galls
					 WHERE $oqey_images.status = 2 
                                           AND $oqey_images.gal_id = $oqey_galls.id 
                                           AND $oqey_images.img_type != 'video'
...
<div class="wrap">
    ...
    <div id="save" style="width:968px; margin-bottom:10px;">
...
<tr id="skin_tr_<?php echo $r->id?>">
    ...
    <td width="630" align="left" valign="top" style="margin-left:10px; padding:5px;">
        ...
        <p><?php echo urldecode($r->description); ?><br/>
             <?php _e('Skin files location', 'oqey-gallery'); ?>: <code>/skins/<?php echo $r->folder; ?></code>.
        </p>
        <p>
...
foreach($galls as $gal){
    ...
    $imgs = $wpdb->get_results("SELECT * FROM $oqey_images WHERE status=2 AND img_type != 'video' AND gal_id = '".absint($gal->gal_id)."' ORDER BY img_order ASC, id DESC");
           ...
           <tr>
            <td width="200" height="50" align="left" valign="middle">
             <div class="dodiv">
              ...
              <input class="doapply button" type="button" name="doapply" value="'.__('Apply', 'oqey-gallery').'"/>
                  </div>
            </td>
	...
	$r .= '<ul class="sortablegalls" style="display:none;">';
        ...
        $splash = $gal->splash_img;

foreach($imgs as $img){
    
    if($img->img_type=="nextgen"){
       $gthmbnew = get_option('siteurl').'/'.trim($img->img_path).'/thumbs/thumbs_'.trim($img->title);
     }elseif($img->img_type=="wp"){
       $gthmbnew = wp_get_attachment_thumb_url( $img->meta_data );
     }else{
       $gthmbnew = oQeyPluginRepoUrl().'/galleries/'.oqey_getBlogFolder($wpdb->blogid).$gal->folder.'/galthmb/'.trim($img->title);
     } 
       ...
       if($img->id == $splash){ 
		...
		<img src="'.$gthmbnew.'" alt="image_'.$img->id.'" class="trashimages"/>
                </li>';
}
    ...
    jQuery.post(ajaxurl, { action:"oQeyImagesFromTrash", imgallfromtrash: encodeURIComponent(names), galid: galid },
        function(data){
           ...
           if(names.length>0){
              ...
              x.find(".messages").hide().html('<?php echo $beforemsg?><?php _e('&nbsp;All selected images was restored.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow");
              var data = eval('(' + data + ')');
              
              setTimeout(function (){
                  if(data.statusul=="no"){ 
                      jQuery("#row_"+galid).fadeOut(); 
                  } 
              }, 1000);
		
            }else{
                
		x.find(".messages").hide().html('<?php echo $beforemsg?><?php _e('&nbsp;Please select an image', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow"); 
		clearUp(); 
    ...
    if(id==3){
        ...
        function(data){
          if(names.length>0){	
            ...
            x.find(".messages").hide().html('<?php echo $beforemsg?><?php _e('&nbsp;All selected images was deleted permanently.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow");
...
jQuery.post(ajaxurl, { action:"oQeyFromTrashVideo", id: id },
   function(data){
			...
			jQuery("#save").hide().html('<?php echo $beforemsg?>' + data.mesaj + '<?php echo $aftermsg?>').fadeIn("slow"); 
   });
}
...
function deleteVideo(id, name){ 
   ...
   function(data){
            jQuery("#video_tr_" + id).fadeOut('slow');
            if(data=="no"){ 
                Query("#tabs-55").fadeOut(); 
            }
            jQuery("#save").hide().html('<?php echo $beforemsg?><?php _e('&nbsp;Video was deleted.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow"); 
   });
...
function deleteSong(id, name){ 
   ...
   function(data){
            ...
            jQuery("#save").hide().html('<?php echo $beforemsg?><?php _e('&nbsp;Song was deleted.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow"); 
   });
...
jQuery.post(ajaxurl, { action:"oQeySongFromTrash", restoresong: id },
   function(data){
            jQuery("#row_" + id).fadeOut('slow');
			if(data=="no"){ jQuery("#tabs-11").fadeOut(); }
			jQuery("#save").hide().html('<?php echo $beforemsg?><?php _e('&nbsp;Song was restored.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow"); 
   });
}
...
jQuery.post(ajaxurl, { action:"oQeySkinPermDelete", deleteskinper: id, name: name },
   function(data){
			...
			if(data=="no"){ jQuery("#tabs-22").fadeOut(); }
                        jQuery("#save").hide().html('<?php echo $beforemsg?><?php _e('Skin was deleted.', 'oqey-gallery'); ?><?php echo $aftermsg?><\/div><\/div>').fadeIn('slow');
...
jQuery.post(ajaxurl, { action:"oQeyGalleryFromTrash", undogallid: id },
   function(data){
			...
			jQuery("#save").hide().html('<?php echo $beforemsg?><?php _e('Gallery was restored.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow"); 
...
jQuery.post(ajaxurl, { action:"oQeyGalleryPermDelete", deletegall: id, name: name },
   function(data){
			...
			jQuery("#save").hide().html('<?php echo $beforemsg?><?php _e('Deleted.', 'oqey-gallery'); ?><?php echo $aftermsg?>').fadeIn("slow"); 
   });
}
